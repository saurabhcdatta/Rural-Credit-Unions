...
BRANCH_FILE <- "data/raw/Credit_Union_Branch_Information.txt"
GEO_DIR     <- "data/geo"
MAP_DIR     <- "output/maps"
...
CAPTION <- paste0(
  "Source: NCUA Credit Union Branch Information, 2026Q1. Headquarters identified by MainOffice = Yes (exactly one per charter);\n",
  "SiteTypeName 'Corporate Office' is not equivalent and would overstate the count. Connecticut mapped to planning regions.\n",
  "Alaska, Hawaii and Puerto Rico rescaled and inset; Guam and the US Virgin Islands omitted for lack of geometry at this scale.\n",
  "Albers equal-area projection. NCUA Office of the Chief Economist.")
...
map_a <- ggplot() +
  ...
  labs(title = "Where America's credit unions are headquartered",
       subtitle = sprintf(
         "%s federally insured credit unions, one point of control each, mapped to the county of their main office.\n%s of %s counties host at least one; %s host none.",
